...
from dataquality.schemas import RequestType, Route, ProcName
...
class ApiClient:
    ...
    def _get_project_run_id(self, project_name: str = None, run_name: str = None) -> Tuple[UUID4, UUID4]:
        ...
        if (project_name and not run_name) or (run_name and not project_name):
            raise GalileoException("You must either provide both a project and run"
            ...
            if not res:
                raise GalileoException(f"No project/run found with name {project_name}/{run_name}")
    ...
    def get_labels_for_run(self, project_name: str = None, run_name: str = None, task: str = None) -> List[str]:
        ...
        project, run = self._get_project_run_id(project_name=project_name, run_name=run_name)
    ...
    def get_tasks_for_run(self, project_name: str = None, run_name: str = None) -> List[str]:
    ...
    def reprocess_run(self, project_name: str = None, run_name: str = None) -> Dict:
        ...
        * Smart features
        ...
        res = self.make_request(
            RequestType.POST,
            url=f"{config.api_url}/{Route.proc_pool}",
            body=body
        )
        print(
