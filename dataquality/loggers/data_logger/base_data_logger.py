...
class BaseGalileoDataLogger(BaseGalileoLogger):
    ...
    def upload(self, last_epoch: Optional[int] = None, data_embs: bool = False) -> None:
        """
        ...
        If data_embs is True, this will also run an off the shelf transformer
        ...
        for split in Split.get_valid_attributes():
            ...
            self.upload_split(
                object_store, in_frame_split, split, split_loc, last_epoch, data_embs
            )
    ...
    def _upload_data_embs(cls, df: DataFrame, split: str, epoch_or_inf: str) -> None:
        ...
        object_store = ObjectStore()
        df_copy = df.copy()
        data_embs = get_data_embs(df_copy)
    ...
    def upload_split(
        ...
        for epoch_or_inf in tqdm(
            ...
            if data_embs and (
                ...
                print(f"Getting data embeddings for {name}")
                cls._upload_data_embs(input_batch, split, epoch_or_inf)
