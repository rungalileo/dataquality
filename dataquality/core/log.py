...
def log_input_data(**kwargs) -> None:
    assert config.task_type, "You must call dataquality.init before logging data"
    data_config = BaseGalileoDataConfig().get_config(task_type=config.task_type)(**kwargs)
    data_config.log()


# Backwards compatibility
def log_batch_input_data(**kwargs) -> None:
    log_input_data(**kwargs)


def log_model_outputs(**kwargs) -> None:
    assert config.task_type, "You must call dataquality.init before logging data"
    model_config = BaseGalileoModelConfig().get_config(task_type=config.task_type)(
        **kwargs)
    model_config.log()
