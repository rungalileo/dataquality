...
def _hf_map_image_feature(
    example: dict, imgs_colname: str, imgs_remote_colname: str
) -> dict:
    ...
    if image["bytes"] is None:
        ...
        example[imgs_remote_colname] = _write_image_bytes_to_objectstore(
            img_path=image["path"],
        )
    else:
        example[imgs_remote_colname] = _write_image_bytes_to_objectstore(
...
def process_hf_image_feature_for_logging(
    dataset: datasets.Dataset, imgs_colname: str, imgs_remote_colname: str
) -> datasets.Dataset:
    ...
    dataset = dataset.map(
        ...
        fn_kwargs=dict(
            imgs_colname=imgs_colname, imgs_remote_colname=imgs_remote_colname
        ),
    )
    return dataset


def _hf_map_image_file_path(
    example: dict, imgs_location_colname: str, imgs_remote_colname: str
) -> dict:
    example[imgs_remote_colname] = _write_image_bytes_to_objectstore(
...
def process_hf_image_paths_for_logging(
    dataset: datasets.Dataset, imgs_location_colname: str, imgs_remote_colname: str
) -> datasets.Dataset:
    dataset = dataset.map(
