...
It is assumed that you successfully installed the dataquality install-server. To
...
The dataquality-server requires a configuration file to run. This is a
...
**WARNING**: The command `vserver` looks in certain directories by default. It is
...
environments:
  test:
    ...
    # other types can be used in future releases of dataquality
    type: test
    ...
    # instance as dataquality makes use of their API (!)
    # OPTIONAL
    vpn_server:
...
settings. The Configure section details
...
**WARNING**: All users that are imported using `vserver import` receive the superuser
role. We are looking into ways to also be able to import roles. For more
background info refer to this
[issue](https://github.com/dataquality/dataquality/issues/71).
...
<details>
<summary><a>Example batch import</a></summary>```yaml
organizations:
...
section in the example configuration in Configuration file structure.
...
**WARNING**: The preferred method of managing entities is using the API, instead of the
...
**NOTE**: Organizations have a public key that is used for end-to-end encryption.
...
**NOTE**: API keys are hashed before stored in the database. Therefore you need to
save the API key immediately. If you lose it, you can reset the API key
later via the shell, API, client or UI.

### Tasks and Results

**WARNING**: Tasks(/results) created from the shell are not picked up by nodes that are
...
**NOTE**: Tasks that share a `run_id` have access to the same temporary folder at
