...
dataquality.login = MagicMock()
dataquality.upload = MagicMock
...
def patch_upload(wait=True):
    ...
    for row in in_out["emb"].values.tolist():
        assert isinstance(row, list), f'Row should be a list, ' \
                                      f'but was {row} of type {type(row)}'
...
def _log_data(num_records=100) -> None:
    ...
    write_output_dir = f"{JsonlLogger.LOG_FILE_DIR}/{dataquality.config.current_project_id}/{dataquality.config.current_run_id}"
