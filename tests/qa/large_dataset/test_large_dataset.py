...
import pandas as pd
import time
from dataquality.core.integrations.config import GalileoDataConfig
...
def download_dataset_from_aws(dataset_folder_path: str) -> None:
    cmd = (
        f"aws s3 sync s3://{dataset_folder_path} ."
    )
    os.system(cmd)
...
if __name__ == '__main__':
    ...
    for epoch_idx in range(NUM_EPOCHS):
        ...
        for i in alive_it(range(0, len(train_dataset), BATCH_SIZE)):
            ...
            dataquality.log_model_outputs(GalileoModelConfig(
                emb=embedding, probs=probs, split="train", epoch=epoch_idx, ids=batch["id"]
            ))
        # Test
        print("Testing")
