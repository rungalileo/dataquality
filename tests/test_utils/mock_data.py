...
class MockDatasetCV(Dataset):
    def __init__(self, root_folder: str = TEST_IMAGES_FOLDER_ROOT) -> None:
        ...
        self.remote_root_folder = "s3://galileo-s3os-images/ImageClassification/dq_tests/"
        self.labels = os.listdir(self.local_root_folder)

        self.dataframe = pd.DataFrame(columns=["image_name", "label"])
        for label in self.labels:
            df_label = pd.DataFrame(
                data=os.listdir(Path(self.local_root_folder) / label), columns=["image_name"]
            )
        ...
        self.dataframe["image_path"] = self.dataframe[["image_name", "label"]].apply(
            lambda r: str(Path(self.local_root_folder) / r["label"] / r["image_name"]), axis=1
        )
        self.dataframe["remote_image_path"] = self.dataframe[["image_name", "label"]].apply(
            lambda r: str(f'{self.remote_root_folder} / {r["label"]} / {r["image_name"]}'), axis=1
        )
