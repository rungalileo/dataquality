...
DATASETS = [
    f"{BUCKET}/datasets/original/newsgroups/newsgroups_train.csv",
    f"{BUCKET}/datasets/original/newsgroups/newsgroups_test.csv",
]
...
def load_dataset_split(dataset: str, split: str) -> pd.DataFrame:
    dataset = pd.read_csv(f"{CWD}/{dataset}_{split}.csv")
...
def test_e2e_text_classification_mock_data() -> None:
    dataquality.login()
